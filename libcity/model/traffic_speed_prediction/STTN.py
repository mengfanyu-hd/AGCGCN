...
class STTN(AbstractTrafficStateModel):
    ...
    def forward(self, batch):
        inputs = batch['X']
        print(inputs.shape)#([64, 12, 207, 1])

        inputs = inputs.permute(0, 3, 2, 1)
        print(inputs.shape)#([64, 1, 207, 12])

        input_transformer = self.conv1(inputs)
        print(input_transformer.shape)#([64, 64, 207, 12])

        input_transformer = input_transformer.permute(0, 2, 3, 1)
        print(input_transformer.shape)#([64, 207, 12, 64])

        output_transformer = self.transformer(input_transformer)
        print(output_transformer.shape)#([64, 207, 12, 64])

        output_transformer = output_transformer.permute(0, 2, 1, 3)
        print(output_transformer.shape)#([64, 12, 207, 64])

        out = self.act_layer(self.conv2(output_transformer))
        print(out.shape)#([64, 12, 207, 64])

        out = out.permute(0, 3, 2, 1)
        print(out.shape)#([64, 64, 207, 12])

        out = self.conv3(out)
        print(out.shape)#([64, 1, 207, 12])

        out = out.permute(0, 3, 2, 1)
        print(out.shape)#([64, 12, 207, 1])
