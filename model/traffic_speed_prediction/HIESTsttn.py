...
class CHGCN(nn.Module):
    def __init__(self, c_in, c_out, dropout, Mor, global_nodes, regional_nodes, device, n1, n2, n3, n4, support_len,
        ...
        self.gcn = GCNHIEST(c_in, c_out, dropout, support_len, order)
...
##################
        self.conv1 = nn.Conv2d(self.nhid, self.embed_dim, 1)
        self.conv11=nn.Conv2d(self.embed_dim, self.nhid, 1)
#################
        ...
        for b in range(self.blocks):
            ...
            for i in range(self.layers):
                ...
                self.conv2.append(nn.Conv2d(self.input_window - 2 * i, self.output_window, 1))

                new_dilation *= 2
                receptive_field += additional_scope
                additional_scope *= 2
                # Add the HGCN for hierarchical graph convolution
                self.gconv.append(CHGCN(self.dilation_channels, self.residual_channels,self.dropout,self.Mor_mx,self.global_nodes,self.regional_nodes,device=self.device,n1=self.n1,n2=self.n2,n3=self.n3,n4=self.n4,support_len=self.supports_len,order=self.order))
                ####################
                self.transformer = Transformer(
                    self.transformer_adj_mx, embed_dim=self.embed_dim, num_layers=self.num_layers,
                ...
                self.conv4 = nn.Conv2d(self.embed_dim, self.nhid, 1)
    ...
    def forward(self, batch):
        ...
        for i in range(self.blocks * self.layers):
            ...
            output_transformer = output_transformer.permute(0, 3, 1, 2)
            #print(output_transformer.shape)
            out = self.act_layer(self.conv11(output_transformer))
            #print(out.shape)
